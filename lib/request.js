...
function ngsiVersion(res, preferVersion) {
    if (preferVersion) {
        const payloadVersion = semver.gt(`${preferVersion}.1`, `${Config.getConfig().version}.1`)
            ...
            : preferVersion;
        res.header('NGSILD-Version', payloadVersion);
        if( res.statusCode === StatusCodes.OK &&
            semver.lt(`${preferVersion}.1`, `${Config.getConfig().version}.1`)){
            res.statusCode = StatusCodes.NON_AUTHORITATIVE_INFORMATION;
        }
...
function linkContext(res, isJSONLD) {
    if (!isJSONLD && is2xxSuccessful(res.statusCode)) {
        res.header(
            'Link',
            `<${Config.getConfig().linkHeader}>; rel="http://www.w3.org/ns/json-ld#context"; type="application/ld+json"`
        );
    }
}
