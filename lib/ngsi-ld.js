...
function formatAttribute(attr, transformFlags = {}) {
    ...
    switch (attr.type.toLowerCase()) {
        ...
        // Temporal Properties
        case 'datetime':
            if (Config.getConfig().valueType && semver.gt(version, '1.9.0')) {
        ...
        case 'time':
            if (Config.getConfig().valueType && semver.gt(version, '1.9.0')) {
                obj.value = moment.tz(attr.value, 'Etc/UTC').format(moment.HTML5_FMT.TIME_SECONDS);
                obj.valueType = 'Time';
            } else {
                obj.value = {
                    '@type': 'Time',
    ...
    if (attr.metadata) {
        ...
        Object.keys(attr.metadata).forEach(function (key) {
            switch (key) {
                ...
                case 'expiresAt':
                case 'dateExpires':
                    if (Config.getConfig().expiresAt && semver.gt(version, '1.9.0')) {
                        obj.expiresAt = moment.tz(attr.metadata[key].value, 'Etc/UTC').toISOString();
                    }
                    break;
                case 'createdAt':
                case 'dateCreated':
...
/**
 * Amends an NGSI-v2 type payload to  NGSI-LD EntityMap format
 *
 */
function formatEntityMap(ids, isJSONLD) {
    const entityMap = {};
    ids.forEach((id) => {
        entityMap[id] = '@none';
    });
    ...
    const obj = {
        id: 'urn:ngsi-ld:EntityMap:' + uuidv4(),
        type: 'EntityMap',
        entityMap,
        linkedMaps: [],
...
exports.formatEntityMap = formatEntityMap;
