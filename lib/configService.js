...
function processEnvironmentVariables() {
    ...
    if (process.env.NGSI_V2_ENTITY_LIMIT) {
        config.limit = process.env.NGSI_V2_ENTITY_LIMIT;
    }
    ...
    if (process.env.LEPUS_MULTI_CORE) {
        config.multiCore = process.env.LEPUS_MULTI_CORE === 'true';
    } else {
    ...
    config.expiresAt = semver.gt(`${config.version}.1`, '1.9.0');
